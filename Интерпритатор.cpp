...
private:
    ...
    vector<variant<int, float>> runtime_stack;

    // Таблица символов (переменных)
    // Хранит имена переменных и их текущие значения (int или float)
    unordered_map<string, variant<int, float>> symbol_table;
    ...
    void push(variant<int, float> val)
    {
        runtime_stack.push_back(val);
    }

    variant<int, float> pop()
    {
        if (runtime_stack.empty())
        {
            throw runtime_error("Runtime Error: Stack underflow.");
        }
        variant<int, float> val = runtime_stack.back();
        runtime_stack.pop_back();
        return val;
    }

    // Вспомогательные функции для операций
    variant<int, float> perform_binary_op(variant<int, float> op1, variant<int, float> op2, OPSCode op_code);
    bool is_false(const variant<int, float> &val);
...
variant<int, float> Interpreter::perform_binary_op(variant<int, float> op1, variant<int, float> op2, OPSCode op_code)
{
        ...
        op_code == OPSCode::OP_GE || op_code == OPSCode::OP_EQ || op_code == OPSCode::OP_NE)
    {
        ...
        if (is_float)
        {
            if (op_code == OPSCode::OP_LS)
                result = f_op1 < f_op2;
            else if (op_code == OPSCode::OP_LE)
                result = f_op1 <= f_op2;
            else if (op_code == OPSCode::OP_GS)
                result = f_op1 > f_op2;
            else if (op_code == OPSCode::OP_GE)
                result = f_op1 >= f_op2;
            else if (op_code == OPSCode::OP_EQ)
                result = f_op1 == f_op2;
            else if (op_code == OPSCode::OP_NE)
                result = f_op1 != f_op2;
            else
...
bool Interpreter::is_false(const variant<int, float> &val)
{
    ...
    while (program_counter < ops_code.size())
    {
        ...
        try
        {
            ...
            case OPSCode::OP_IDENT:
            {
                ...
                {
                    push(symbol_table[var_name]);
                }
                else
                {
                    // Для ID в качестве операнда (для чтения значения)
                    // Это означает, что переменная не была инициализирована
                    throw runtime_error("Runtime Error: Undefined variable '" + var_name + "'.");
                }
                break;
            }
            ...
            case OPSCode::OP_DIV:
            {
                variant<int, float> op2 = pop();
                variant<int, float> op1 = pop();
            ...
            case OPSCode::OP_JF:
            {
                ...
                variant<int, float> condition_result = pop();
            ...
            {
                variant<int, float> value_to_assign = pop();
                // Ожидаем, что следующая инструкция - это OP_IDENT с именем переменной
                if (runtime_stack.empty() || !holds_alternative<string>(runtime_stack.back()))
                {
